...
class TravelingSalesmanProblem:
    ...
    def successors(self):
        ...
        Cities=self.copy()  # Type = <class '__main__.TravelingSalesmanProblem'>

        NumCities=len(Cities.path)
        ...
        for i in range(NumCities):
            ...
            if i+1 not in range(NumCities):
                newlist=Cities.copy()
                newlist.path[0],newlist.path[i]=newlist.path[i],newlist.path[0]
                AllNeigbs.append(newlist)
                return AllNeigbs
            else:
                newlist=Cities.copy()
                newlist.path[i],newlist.path[i+1]=newlist.path[i+1],newlist.path[i]
                AllNeigbs.append(newlist)
    ...
    def get_value(self):
        ...
        for i in range(NumCities):
            if i==0:
                Total_Distance=Total_Distance+np.sqrt((thecoords[NumCities-1][0]-thecoords[0][0])**2+(thecoords[NumCities-1][1]-thecoords[0][1])**2)
            elif i+1 not in range(NumCities):
                return -Total_Distance
            Total_Distance=Total_Distance+np.sqrt((thecoords[i][0]-thecoords[i+1][0])**2+(thecoords[i][1]-thecoords[i+1][1])**2)
